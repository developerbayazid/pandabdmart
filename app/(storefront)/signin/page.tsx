...
function SignInForm() {
    ...
    const handleOAuthSignIn = async (provider: 'google' | 'github') => {
        ...
        const { error: authError } = await supabase.auth.signInWithOAuth({
            provider,
            options: { redirectTo: `${origin}/api/auth/callback?redirect=${encodeURIComponent(redirectTo)}` },
        });
                    ...
                    <>
                        ...
                        <div className="space-y-3">
                            <button
                                ...
                                disabled={loading}
                                className="w-full bg-surface border border-border rounded-md px-4 py-2 text-sm font-medium text-text-primary hover:bg-surface-secondary disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
                            >
